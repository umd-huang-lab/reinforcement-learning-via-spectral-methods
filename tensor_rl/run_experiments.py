...
from tensor_rl.planning import ValueIteration
from tensor_rl.experiments import Experiment
from tensor_rl.mdp import MarkovGameMDP
from tensor_rl.utils import chart_utils
from tensor_rl.agents import *
from tensor_rl.tasks import *
...
def run_agents_on_mdp(agents,
    ...
    if track_success and success_reward is None:
        raise ValueError("(tensor_rl): run_agents_on_mdp must set param @success_reward when @track_success=True.")
...
def run_single_agent_on_mdp(agent, mdp, episodes, steps, experiment=None, verbose=False, track_disc_reward=False, reset_at_terminal=False, resample_at_terminal=False):
    ...
    if reset_at_terminal and resample_at_terminal:
        raise ValueError("(tensor_rl) ExperimentError: Can't have reset_at_terminal and resample_at_terminal set to True.")
...
def reproduce_from_exp_file(exp_name, results_dir="results", open_plot=True):
    ...
    if not os.path.exists(full_exp_file):
        raise ImportError("(tensor_rl): no such experiment: " + str(full_exp_file) + ".")

    # Open the file.
    import json
    from tensor_rl.utils.additional_datastructures import TupleEncoder
...
def choose_mdp(mdp_name, env_name="Asteroids-v0"):
    ...
    # Other imports
    from tensor_rl.tasks import ChainMDP, GridWorldMDP, FourRoomMDP, TaxiOOMDP, RandomMDP, PrisonersDilemmaMDP, RockPaperScissorsMDP, GridGameMDP
    ...
    if mdp_name == "gym":
        # OpenAI Gym MDP.
        try:
            from tensor_rl.tasks.gym.GymMDPClass import GymMDP
        except:
            raise ValueError("(tensor_rl) Error: OpenAI gym not installed.")
...
def main():
    ...
    # Setup agents.
    from tensor_rl.agents import RandomAgent, QLearningAgent
